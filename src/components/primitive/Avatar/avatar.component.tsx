...
import { NormalSizes } from './avatar.component.props';
...
const Avatar: React.FC<AvatarProps> = ({ src, stacked, text, size, isSquare, className, ...props }) => {
  const showText = !src;
  const radius = isSquare ? '50%' : '50%';
  const marginLeft = stacked ? '-.625rem' : 0;
  const width = useMemo(() => getSize(size), [size]);

  return (
    <span className={`avatar ${className}`}>
      {!showText && <img className='avatar-img' src={src} alt='avatar' {...props} />}
      {showText && (
